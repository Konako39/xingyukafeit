...
from deepseek_gateway import call_background_preferred
...
def _quarantine_legacy_grounding_errors(connection: sqlite3.Connection) -> int:
    ...
    for row in experiences:
        ...
        elif source_type.startswith("screen_") and (
            _grounding_violation(content, "screen")
            or re.search(
                r"(?:可能|大概|多半|似乎|看起来).{0,30}"
                r"(?:整理|担心|准备|打算|想|看完|正在|刚)"
                r"|是不是.{0,30}[？?]|笑得前仰后合",
                content,
            )
        ):
            experience_issue = "旧屏幕记忆含有截图无法证明的动作、动机或心理推测"
...
def _screen_prompt(
    ...
    recent = recent_persona_experiences(
        connection, persona, limit=4, max_chars=1_200
    )
    recent_text = format_persona_experiences(persona, recent) if recent else "(暂无)"
    return (
        ...
        "只写画面直接显示的应用、窗口、文件和可辨认文字；不要猜主人正在整理、"
        "担心、准备或打算做什么，也不要推断心理、动机和屏幕外事件。"
        "能读到账号、密钥或其他细节时可如实记录，但绝不能编造看不清的文字。"
        "画面操作者始终写成‘主人’，禁止使用‘你正在’‘用户正在’或把操作写成"
        "艾莉/沙雅做的。不要向主人说话，不要给建议，不要写报告标题、Markdown"
        "或列表；只输出一段80到220字、以后适合语义检索的第一人称经历。"
        f"观察时间：{captured_at}\n\n"
        f"你自己记忆池里最近的经历（仅用于识别连续性）：\n{recent_text}"
    )
    ...
    try:
        ...
        cursor = connection.execute(
            ...
            (
                captured_at,
                json.dumps(
                    {**(image_metadata or {}), "resources": snapshot, "image_retained": False},
                    ensure_ascii=False,
                ),
            ),
        )
        ...
        interval = random.randint(
            int(config["screen_min_interval_minutes"]),
            int(config["screen_max_interval_minutes"]),
        )
        next_run = (
        ...
        connection.execute(
            ...
            UPDATE lounge_config
             ...
             WHERE id = 1
            """,
            (finished, next_run, f"已观察，下次最早 {interval} 分钟后", finished),
        )
        connection.commit()
        return {
            "completed": True,
    ...
    try:
        ...
        while True:
            ...
            if turn == 1 and resume_transcript:
                ...
                turn_hint = (
                    "自然接住上一条，最多推进一个新想法；不需要为了延长聊天硬换题，"
                    "觉得说够了也可以像普通网友一样短短收住。"
                )
            mode_hint = (
...
def lounge_history(
    ...
    ensure_lounge_schema(connection)
    sessions = connection.execute(
        "SELECT * FROM lounge_sessions ORDER BY id DESC LIMIT ?",
        (max(1, min(limit, 50)),),
    ).fetchall()
...
def screen_watch_history(
    ...
    ensure_lounge_schema(connection)
    rows = connection.execute(
        "SELECT * FROM screen_observations ORDER BY id DESC LIMIT ?",
        (max(1, min(limit, 50)),),
    ).fetchall()
    result: list[dict[str, object]] = []
    for row in rows:
...
def lounge_payload(connection: sqlite3.Connection) -> dict[str, object]:
    ...
    screen_running = bool(screen_rows and screen_rows[0]["status"] == "running")
    with SCREEN_REQUEST_LOCK:
