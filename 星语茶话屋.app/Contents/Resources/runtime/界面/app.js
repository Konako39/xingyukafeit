...
function updateModelBadge() {
  ...
  $('#profileGlimpse').textContent = state.settings.persona === 'aili'
    ? '想聊什么就直接来找我吧。偶尔会逗逗你，但真要做事时，我可是很靠谱的哦💗'
    : '按部就班、认真细致是我的风格。被夸的时候或许会有点不好意思……但事情一定会替你稳稳做好。';
  $('#welcomeDescription').textContent = state.settings.persona === 'aili'
    ? '什么话题都找我吧💗 聊天、创作，还是认真做事，我都在。'
    : '认真可靠是我的原则。交给我吧，我会先把事情弄清楚，再稳稳陪你处理好。';
}
...
function qualityLabel(mode) { return ({fast:'快速', balanced:'均衡', deep:'深度'})[mode] || '均衡'; }

function metricsHTML(metadata = {}) {
  const parts = [];
  if (metadata.quality_mode) parts.push(qualityLabel(metadata.quality_mode));
...
function renderSessions() {
  ...
  const items = state.conversations.filter(item => !query || `${item.title} ${item.preview}`.toLowerCase().includes(query));
  $('#sessionCount').textContent = state.conversations.length;
  $('#sessionList').innerHTML = items.length ? items.map(item => `
    ...
    </button>`).join('') : '<div class="empty-sessions">还没有对话<br>点击上方开始</div>';
...
async function sendMessage() {
  ...
  try {
    const response = await fetch('/api/gui/chat', {
      ...
      body:JSON.stringify({conversation_id:state.current.id, persona:state.settings.persona, model:state.settings.model, message:text, images:outgoingImages, temperature:state.settings.temperature, quality_mode:state.settings.qualityMode, top_p:state.settings.topP, repeat_penalty:state.settings.repeatPenalty, seed:state.settings.seed, keep_alive:'1m', max_tokens:state.settings.maxTokens || undefined, client_surface:'main_panel'}),
    });
...
async function refreshConversations() {
  const data = await api('/api/gui/conversations'); state.conversations = data.data;
...
function renderLoungeLog(history=[]){
  ...
  log.innerHTML=history.map(session=>{
    ...
    const dialogue=session.messages?.length?`<div class="lounge-dialogue">${session.messages.map(item=>`<div class="lounge-line ${item.speaker}"><img src="${avatarURL(item.speaker)}" alt=""><div><p>${escapeHTML(item.content)}</p><small>${escapeHTML(formatTime(item.created_at))} · ${escapeHTML(item.model)}${item.metadata?.fallback?' · 同人格备用模型':''}</small></div></div>`).join('')}</div>`:'';
...
$$('#qualitySwitch button').forEach(button=>button.onclick=()=>{if(state.generating)return;state.settings.qualityMode=button.dataset.quality;localStorage.setItem('localai.qualityMode',state.settings.qualityMode);updateQualityControls();toast(`已切换到${qualityLabel(state.settings.qualityMode)}模式`,'success');});
